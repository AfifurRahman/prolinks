...
class DocumentController extends Controller
{
                ...
                if ($file !== '.' && $file !== '..') {
                    ...
                    if (is_dir($filePath)) {
                        ...
                        if(UploadFile::where('basename', $file)->value('status') == '1') {
                            ...
                            $html .= '<div class="items"><div><li><span class="file"><img class="file-icon" src="' . url('template/images/icon_menu/' . $fileExtension . '.png') . '" />' . $index.'&nbsp;'. $fileName . '</span></div><div><input type="checkbox" id="'. $file .'" onchange="setFile(this)" checked></input></div></div></li>';
                        }
                    }
                }
            }
    ...
    public function CheckGroupPermission(Request $request) {
        try { 
    ...
    public function CreateWatermark($fileID)
    {
        ...
        $image = imagecreatefrompng($watermarkpath);

        $width = imagesx($image);
        $height = imagesy($image);
       
        $rotatedImage = imagecreatetruecolor($width, $height);
        imagesavealpha($rotatedImage, true);
        $transparent = imagecolorallocatealpha($rotatedImage, 255, 255, 255, 127);
    ...
    public function Upload(Request $request) 
    {
        try {
            if(Auth::user()->type == \globals::set_role_collaborator() OR Auth::user()->type == \globals::set_role_administrator()) {
                ...
                $receiver_email = AssignProject::where('subproject_id', $subproject_id)->get();
